...
export const getTransactionsByUser = async (userId: string): Promise <Transaction[]> => {
  ...
  return found.map(({ id, userId, type, amount, category, date, comment }) => ({
    id,
    userId,
    type,
    amount,
...
export const createTransaction = async (transaction: Omit<Transaction, 'id'>): Promise <Transaction | null> => {
  ...
  const { id, userId, type, amount, category, date, comment } = created.toObject();
  return { id, userId, type, amount, category, date, comment: comment === null ? undefined : comment };
}
...
export const updateTransaction = async (id: string, updates: Partial<Transaction>): Promise <Transaction | null> => {
  ...
  const { id: txId, userId, type, amount, category, date, comment } = updated.toObject();
  return { id: txId, userId, type, amount, category, date, comment: comment === null ? undefined : comment };
}
